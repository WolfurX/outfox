---
name: taste-skill
description: Anti-slop frontend design skill (vendored from leonxlnx/taste-skill, MIT). Use when building or reworking any player-facing UI surface — typography, spacing, color discipline, interactive states, motion restraint, AI-tell purge. In this repo, docs/DESIGN-SYSTEM-WEB.md OVERRIDES this skill wherever they conflict.
---

<!-- ============================================================================
PROJECT ADAPTATION PREAMBLE (Outfox) — read before the vendored skill below.
Source: https://github.com/leonxlnx/taste-skill @ 06d6028 (MIT — LICENSE alongside).

Precedence in this repo: docs/DESIGN-SYSTEM-WEB.md and docs/THEME-OUTFOX.md are
CANONICAL and win every conflict with this skill. Concretely:
 1. STACK: no Tailwind, no Motion/GSAP, no icon libraries, no Next.js. This project
    uses plain CSS custom-property tokens (tokens.css is the only hex file), system
    font stack + one ≤30KB mono display font, inline SVG sprite icons, and the
    170KB critical-path CI gate. The skill's Section 2/3 stack guidance does not apply.
 2. TOKENS & THEMES: the §4 calibration palettes (dark canonical + light) are fixed;
    the skill's color rules apply to USAGE (consistency lock, one accent, contrast),
    never to VALUES.
 3. COPY: player-facing vocabulary comes exclusively from DESIGN-SYSTEM-WEB Appendix A;
    canonical fixed strings (e.g. the §12 Unsettled explainer) keep their exact text —
    including em-dashes — overriding this skill's Section 9.G for those strings only.
    New copy written under this skill avoids em-dashes per 9.G.
 4. MOTION: DESIGN-SYSTEM-WEB §8 governs (120/200/320ms, transform/opacity only,
    reduced-motion + animation-off toggle, constant-duration RewardReveal, no
    slot-machine theatrics). The skill's motion vocabulary is used only within that.
 5. SCOPE: this is a dense product UI. Per the skill's own Section 13, apply the
    transferable craft rules (typography/spacing/color discipline, interactive states,
    shape lock, AI-tell purge, redesign levers 1-4) — not the landing-page sections.
============================================================================ -->
